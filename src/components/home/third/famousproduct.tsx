...
function FamousProduct() {
  ...
  return (
    <section className="pt-[30px] md:pt-[60px]">
      <div className="test_wrapper container flex flex-col md:flex-row items-center gap-6 md:gap-[80px] bg-[url('/imgs/background.png')] bg-no-repeat bg-cover max-w-[1275px] w-full h-auto md:h-[594px] p-6 md:p-[35px_48px] rounded-[20px] md:rounded-[377px_0px_0px_377px] shadow-lg mx-auto">
        {/* Image Container */}
        <div className="w-[200px] h-[200px] md:w-[543px] md:h-[523px] flex items-center justify-center rounded-full bg-white shadow-lg">
          <Image
            loading="lazy"
            src={selectedImage} // Update main image dynamically
            width={319}
            height={377}
            alt="shirt"
        ...
        {/* Content Container */}
        <div className="flex flex-col gap-4 md:gap-6 items-center md:items-start text-center md:text-left w-full max-w-[450px]">
          <button className="bg-primaryColor rounded-[5px] text-white w-[200px] md:w-[238px] py-2 md:py-[9px] px-4 md:px-[41px] font-gilroy text-base md:text-lg font-medium text-center">
            {b("fmsproduct")}
          </button>
          ...
          <MyButton />
          <div className="flex gap-4 md:gap-[20px] mt-4 md:mt-[30px]">
